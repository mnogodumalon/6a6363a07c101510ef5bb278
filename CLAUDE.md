...
If the user asks for a public page (public form, booking page, public list, landing/submission page — anything anonymous visitors reach via a shared link), use the **`public-builder`** skill. It covers the declaration file (`_public/surface.json`), the registry markers in `src/pages/public/registry.tsx`, `PublicShell`, `publicClient`, and the upgrade path for existing public forms (`_agent_context/public_pages.json`). Public pages never import `livingAppsService`.
