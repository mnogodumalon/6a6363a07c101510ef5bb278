...
import type { AusleiheRueckgabe, Mitarbeiterverwaltung, Werkzeugverwaltung, LookupValue } from '@/types/app';
...
interface AusleiheRueckgabeDialogProps {
  ...
  defaultValues?: Omit<AusleiheRueckgabe['fields'], 'zustand_rueckgabe'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  zustand_rueckgabe: LOOKUP_OPTIONS['ausleihe_&_rueckgabe']?.['zustand_rueckgabe'] ?? [],
};
...
export function AusleiheRueckgabeDialog({ open, onClose, onSubmit, defaultValues, recordId, mitarbeiterverwaltungList, werkzeugverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: AusleiheRueckgabeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['mitarbeiter', 'werkzeug', 'ausleihdatum'] as const;
  ...
  }), [mitarbeiterverwaltungList, werkzeugverwaltungList, ]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'ausleihe_&_rueckgabe');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitarbeiter", "werkzeug"]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'geplante_rueckgabe': (
      <div key="geplante_rueckgabe" className="space-y-1.5">
        ...
        <DatePicker
          ...
          placeholder="Geplantes Rückgabedatum"
          mode="date"
    ...
    'einsatzort': (
      <div key="einsatzort" className="space-y-1.5">
        ...
        <Input
          id="einsatzort"
          placeholder="z. B. Baustelle Hauptstr. 42"
      ...
      <div key="zustand_rueckgabe" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zustand_rueckgabe) === 'leichte_gebrauchsspuren'}
            onClick={() => setFields(f => ({ ...f, zustand_rueckgabe: (lookupKey(f.zustand_rueckgabe) === 'leichte_gebrauchsspuren' ? undefined : 'leichte_gebrauchsspuren') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zustand_rueckgabe) === 'leichte_gebrauchsspuren'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zustand_rueckgabe) === 'verloren'}
            onClick={() => setFields(f => ({ ...f, zustand_rueckgabe: (lookupKey(f.zustand_rueckgabe) === 'verloren' ? undefined : 'verloren') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zustand_rueckgabe) === 'verloren'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkungen"
          placeholder="Schäden, Besonderheiten, Notizen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"mitarbeiter": "Mitarbeiter", "werkzeug": "Werkzeug", "ausleihdatum": "Ausleihdatum", "geplante_rueckgabe": "Geplantes Rückgabedatum", "einsatzort": "Einsatzort / Baustelle", "rueckgabedatum": "Rückgabedatum", "zustand_rueckgabe": "Zustand bei Rückgabe", "bemerkungen": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitarbeiter": {"vorname": "Vorname", "nachname": "Nachname", "personalnummer": "Personalnummer", "abteilung": "Abteilung", "telefon": "Telefonnummer", "email": "E-Mail-Adresse"}, "werkzeug": {"bezeichnung": "Bezeichnung", "inventarnummer": "Inventarnummer", "kategorie": "Kategorie", "hersteller": "Hersteller", "modell": "Modell", "seriennummer": "Seriennummer", "kaufdatum": "Kaufdatum", "naechste_pruefung": "Nächste Prüfung", "standort": "Standort / Lagerort", "zustand": "Zustand", "anmerkungen": "Anmerkungen", "foto": "Foto des Werkzeugs"}};
